        ...
        for name, benchmarker in benchmarkers.items():
            ...
            try:
                ...
                time_taken = time_execution(
                    lambda: benchmarker.evaluate(expr),
                    iterations
                )
...
def print_summary(results: Dict[str, BenchmarkResult], benchmark_cases: List[str]):
    ...
    sorted_by_overall = sorted(
        libs_with_results,
        key=lambda lib: results[lib].get_overall_median()
    )
    ...
    if sorted_by_overall:
        ...
        for lib in sorted_by_overall:
            ...
            print(f"{lib:20} {overall_median:.4f} ms ({relative_speed:.2f}x) - {case_count}/{len(benchmark_cases)} cases")
    else:
        print("No libraries have benchmark results.")


def generate_markdown_table(results: Dict[str, BenchmarkResult], benchmark_cases: List[str]):
    """Generate a markdown table for the README."""
    print("\n\n## Benchmark Results for README")
    print("```markdown")
    print("## ⚡ Performance Benchmarks")
    print("")
    print("PyEvalExpr was benchmarked against several popular Python expression evaluation libraries:")
    print("")
    ...
    print("")
    print("*Benchmarks performed on AMD Ryzen 9 7950X with 32GB RAM running Python 3.11 using 50,000 iterations per test. Lower is better.*")
    print("```")
